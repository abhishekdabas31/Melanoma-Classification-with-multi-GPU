...
def train_fold(fold):
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset,batch_size=512,shuffle=True,sampler=None)
    valid_loader = torch.utils.data.DataLoader(valid_dataset, batch_size=512, shuffle=False,sampler=None)
...
#       predictions = np.array(final_predictions)
    ...
    torch.save(mymodel.state_dict(), 'resnet152_gpu3_512EPOCH_{}.pth'.format(i))   